...
def active(teta , target):
    if target > teta :   return 1
    elif target < -teta :   return -1
    else: return 0
def MLperceptron(instanceDS):
    flag = True
    ...
    while flag == True:
        ...
        for i in range(len(instanceDS)):
            ...
            for j in range(Weight.shape[1]):
                ...
                Fy_in.append(active(teta , target))
            for j in range(Weight.shape[1]):
                if Fy_in[j] != t[i][j]:
                    flag = True
...
def HidL(num,target1):
    m=[]
    for i in range(1,num):
        m.append(random.randint(0,i))
        continue
    return active(0,target1) * (1 - active(0,target1))
...
def ValidateMethod(instanceDS,p):
    ...
    print ('  and in the Test set = {}'.format(err / instanceDS.__len__() ))
...
for teta in tetaSet:
    ...
    B = np.zeros(7)
    MLperceptron(instanceDS)
    learnMethod(instanceDS)
    ValidateMethod(ValidateDS,p);p-=2

fig = plt.figure()

plt.title('hidden layer  = 20  test err = blue line')
plt.yticks(range(1),'')
...
ax2.plot(tetaSet , 2/np.array(ErrTest) , 'b')
